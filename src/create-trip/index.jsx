...
const CreateTrip = () => {
  ...
  const startGoogleLogin = useAppGoogleLogin({
  ...
  async function handleGoogleAuthSuccess(normalizedUser) {
    ...
    toast.success(
      `Welcome ${normalizedUser?.name || "traveler"}! You can now generate your trip.`,
      {
        position: "top-center",
        autoClose: 2200,
      },
      ...
      <main className="mx-auto mt-6 min-h-screen max-w-6xl px-4 pb-16 sm:mt-10 sm:px-6 lg:px-8">
        ...
        <section className="mt-8 rounded-2xl border border-sky-100 bg-white/90 p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {currentUser?.email ? (
              <LogoutButton className="border-slate-300" onLogout={handleLogout} />
            ) : (
              <Button
                variant="outline"
